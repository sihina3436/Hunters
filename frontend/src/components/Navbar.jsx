    ...
    <header className=" fixed top-0 left-0 w-full bg-white shadow-md z-50">
      <nav className="max-w-screen-2xl mx-auto px-4 flex items-center justify-between py-4">
        ...
        <div className="flex-1 flex justify-center">
          <Link to="/" className="text-2xl font-custom font-extrabold text-text-dark">
        ...
        <div className="flex items-center flex-1 justify-end gap-6">
          <Link to='/search' className="text-[20px] text-text-dark hover:text-primary">
            <i className="ri-search-line"></i>
          </Link>
          <button className="hover:text-primary relative">
            <i className="ri-shopping-bag-line"></i>
            <sup className="text-sm px-1.5 text-white rounded-full bg-primary">0</sup>
          </button>
          <Link to='/login' className="text-[20px] text-text-dark hover:text-primary">
      ...
      >
        <ul className="flex flex-col items-center gap-4">
          ...
          <li className="font-medium text-text-dark hover:text-primary cursor-pointer">
            ...
            categories <i className={`ri-arrow-${mobileDropdownOpen ? 'up' : 'down'}-s-line`}></i>
            </div>
            {mobileDropdownOpen && (
              <ul className="mt-2 w-full bg-gray-100 rounded-md py-2">
                <li className="px-4 py-2 hover:bg-gray-200">
                  <Link to="/gift" onClick={() => setMenuOpen(false)}>Gift Collection</Link>
                </li>
                <li className="px-4 py-2 hover:bg-gray-200">
                  <Link to="/bags" onClick={() => setMenuOpen(false)}>Bags Collection</Link>
                </li>
                <li className="px-4 py-2 hover:bg-gray-200">
